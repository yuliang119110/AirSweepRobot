...
演示无人机搭载 SO-ARM101 机械臂，从起点向前平移 3 米穿越门框。

### 7.1 场景描述

| 元素 | 说明 |
|------|------|
| 无人机+机械臂 | `blander/drone_with_arm/drone_with_arm.urdf` |
| 门框 | `blander/base_basic_pbr(2).usdz` |
| 起点 | `(0, 0, 1.05)` 高度 1.05m |
| 平移 | 向前 3 米，至 `(3, 0, 1.05)` |
| 门框位置 | `(1.5, 0, 0)` 位于起点和终点中间 |
| 帧率 | 30 FPS，共 300 帧（约 10 秒） |

### 7.2 启动

```bash
cd /home/dgx/air/test3.2.7

# GUI 模式，跑一次（300 帧后自动退出）
bash fly/run_translate_forward_3_2_7.sh

# GUI 模式，循环跑
LOOP=1 bash fly/run_translate_forward_3_2_7.sh

# 无画面模式（headless）
HEADLESS=1 bash fly/run_translate_forward_3_2_7.sh
```
...
```bash
# 平移 5 米，偏航 45°，headless 循环
YAW_DEG=45 FORWARD_METERS=5 HEADLESS=1 LOOP=1 bash fly/run_translate_forward_3_2_7.sh

# 调整飞行高度和门框位置
START_Z=1.2 BASE_X=2.0 bash fly/run_translate_forward_3_2_7.sh
```
...
```bash
# 查看日志
docker logs -f isaac_translate_forward

# 停止仿真
docker stop isaac_translate_forward
```

### 7.5 首次启动说明

在 Jetson Orin（aarch64）上首次启动 Docker 容器时，Isaac Sim 需要编译 RTX 光线追踪着色器，分两个阶段：

```
Phase 1: RtPso async compilation          → 约 140 秒
Phase 2: GPU driver ray tracing shaders   → 约 100 秒
总共约 4 分钟
```

之后同一容器不会重复编译，但新容器仍需重新编译。如需快速验证可加 `HEADLESS=1` 跳过渲染管线初始化中的 GUI 部分。
